...
{int i,numReg;
 ...
 char dni[10];
  ...
  else { printf("Registro con NRR %d\n",i);
  ...
  //Fin de B�squeda
  printf("Introducimos un nuevo alumno\n");
  	strcpy(reg2.nombre,"Nuriiiii\0");
	strcpy(reg2.dni,"34002141\0");
	strcpy(reg2.ape1,"Salvo\0");
	strcpy(reg2.ape2,"Mateos\0");
	strcpy(reg2.provincia,"Cáceres\0");
	i=insertaReg("alumnos.dat", &reg2);
	printf("Se introducio en el NRR %d\nVeamos la lista:\n",i);
  numReg=leeSecuencial("alumnos.dat");
  printf("Numero de Registros: %d \n\n\n",numReg);
